...
func _run() -> void:
	...
	var mode_manager: Node = scene.get_node_or_null("GameModeManager")
	...
	_assert(mode_manager != null, "存在 GameModeManager")
	...
	if game_state != null:
		...
		_assert(not mode_manager.is_survival_mode(), "默认进入战役模式")
		mode_manager.start_survival_mode(true)
		_assert(mode_manager.is_survival_mode(), "可以启动生存模式")
		mode_manager.notify_wave_started(6)
		mode_manager.notify_wave_cleared(6)
		_assert(mode_manager.survival_highest_wave == 6 and mode_manager.survival_cleared_waves == 6, "生存模式会记录最高波和清理波数")
		var campaign_composition: Dictionary = wave_manager._build_wave_composition(5)
		var survival_composition: Dictionary = wave_manager._build_wave_composition(6)
		_assert(int(survival_composition.get("elite", 0)) >= 1, "生存高波次会追加精英敌人")
		wave_manager._wave = 6
		_assert(wave_manager._get_current_spawn_interval() < wave_manager.spawn_interval, "生存模式会缩短刷怪间隔")
		mode_manager.set_campaign_mode()
		_assert(not mode_manager.is_survival_mode(), "可以切回战役模式")
		_assert(not campaign_composition.is_empty(), "战役波次组合仍然可用")
